...
use voicebot::control_client::{ControlClient, ClientControlEvent};
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Current state: {} (utterance_id: {:?})", state.state, state.utterance_id);
        ...
        match tokio::time::timeout(Duration::from_millis(100), events_rx.recv()).await {
            Ok(Some(event)) => {
                ...
                if !complete_response.is_empty() && !search_results.is_empty() {
                    ...
                    if complete_response.to_lowercase().contains("verstappen") 
                        ...
                        println!("   No specific driver names found - may need to check search results");
                    }
            ...
            Err(_) => {
                if !complete_response.is_empty() {
                    ...
                    println!("\nPartial Results:");
                    println!("   - LLM response: {} characters", complete_response.len());
                    println!("   - Response: {}", &complete_response[..complete_response.len().min(300)]);
                    return Ok(());
                }
            }
        }
