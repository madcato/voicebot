...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {"task": {"type": "string", "description": "Full description of the task to delegate"}},
                "required": ["task"],
            },
        },
    },
...
def _post_stream(host, port, token, payload):
    ...
    try:
        conn.request("POST", "/v1/chat/completions", body=body, headers=_auth_headers(token))
...
def load_model(host, port, token, model_id):
    ...
    _post_blocking(host, port, token, {
        "model":      model_id,
...
def measure_pp(host, port, token, model_id, runtime_name):
    ...
    payload = _base_speed_payload(model_id, max_tokens=GEN_TOKENS, stream=True, runtime_name=runtime_name)
...
def run_mechanical_checks(fixture: dict, text: str, tool_calls: list) -> list[tuple]:
    ...
    for tool_name, expected_arg in ev.get("tool_args_contain", {}).items():
        ...
        if not matching:
            checks.append(("tool_args_contain", False,
            ...
            checks.append(("tool_args_contain", passed,
...
def call_evaluator(ev_cfg: dict, fixture: dict, fixture_messages: list,
                   text: str, tool_calls: list, mech: list) -> dict:
    """
    ...
    for msg in fixture_messages:
        ...
        if msg.get("tool_calls"):
            ...
            conv_lines.append(f"{role}: [tool call: {fn['name']}({fn.get('arguments','')[:80]})]")
        elif role == "tool":
    ...
    if failed_mechs:
        mech_note = "\nMechanical checks FAILED:\n" + "\n".join(f"  - {d}" for d in failed_mechs)
    elif mech:
    ...
    resp = _post_blocking(ev_cfg["host"], ev_cfg["port"], ev_cfg["token"], {
        "model":       ev_cfg["model"],
        "messages":    [
...
def run_quality_benchmark(
    ...
    for i, fx in enumerate(fixtures, 1):
        ...
        try:
            ...
            tag = ("tools:" + "+".join(tc["function"]["name"] for tc in tcs)) if tcs else f"{len(text)} chars"
            print(f"ok  ({tag})")
    ...
    ev_available = bool(ev_cfg and _wait_ready(ev_cfg["host"], ev_cfg["port"], ev_cfg["token"], timeout=5))
    ...
    for i, (fx, text, tcs, error) in enumerate(collected, 1):
        ...
        if error:
            print("ERROR")
            results.append({"id": fid, "group": group, "verdict": "ERROR",
                             "reason": error[:120], "mech_pass": False, "preview": ""})
            continue
...
def run_speed_benchmark(host, port, token, model_id, runtime_name, label, W) -> dict | None:
    ...
    try:
        cold_ttft, pp_tps, prompt_tokens = measure_pp(host, port, token, model_id, runtime_name)
...
def print_speed_results(all_results: list, W: int):
    ...
    if len(all_results) >= 2:
        ...
        if no_cache:
            ...
            for r in no_cache:
                print(f"    • {r['label']}  (cold {r['cold_ttft']:.0f} ms → warm {r['ttft']:.0f} ms  {r['speedup']:.1f}×)")
        else:
            print()
...
def print_quality_results(all_results: list, W: int):
    ...
    for r in all_results:
        quality   = r.get("quality", [])
        failures  = [qr for qr in quality if qr["verdict"] in ("FAIL", "PARTIAL", "ERROR")]
        if not failures:
            continue
        any_failure = True
        display = f"{r['server']}/{r['runtime']}  {r['model_id']}"
...
def main():
    ...
    fixtures_path = os.path.join(os.path.dirname(os.path.abspath(config_path)), "fixtures.json")
    ...
    print(f"  Quality     : {len(fixtures)} fixtures" if fixtures else "  Quality     : disabled")
    if ev_cfg:
        ev_ready  = _wait_ready(ev_cfg["host"], ev_cfg["port"], ev_cfg["token"], timeout=3)
        ev_status = f"{ev_cfg['model']}  @ {ev_cfg['host']}:{ev_cfg['port']}"
        ev_status += "  ✓" if ev_ready else "  ✗ unreachable (will fall back to mechanical checks)"
    ...
    for tgt in targets:
        ...
        for i, target_model in enumerate(models, 1):
            ...
            # ── Speed benchmark ──────────────────────────────────────────────
            result = run_speed_benchmark(host, port, token, model_id, runtime_name, label, W)
            if not result:
                continue
